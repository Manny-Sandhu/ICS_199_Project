...
{
    $q = "SELECT item_id, item_name, item_desc, item_picture, item_price, disable_item FROM menu_items where disable_item = 'N';";
} else { #// filtered by category
    $q = "SELECT item_id, item_name, item_desc, item_picture, item_price, disable_item 
...
if ($num > 0) { // If it ran OK, display the records.
    ...
    while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
        ...
        $disabled = $row['disable_item'];
        array_push($id_array, $id);

        // Item is displayed if its not disabled
        if ($disabled == 'N') {
